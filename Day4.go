...
func main() {
	var input string = `kvvfl kvvfl olud wjqsqa olud frc
...
rvbu czwpdit vmlihg spz lfaxxev zslfuto oog dvoksub`
	...
	var list []string = strings.Split(input,"\n")
	var counter int = 0
	for i := 0; i < len(list); i += 1 {
		var found bool = false
		var line []string = strings.Split(list[i]," ")
		for j := 0; j < len(line); j += 1 {
			line[j] = SortString(line[j])
		}
		dup_map := dup_count(line)
		...
		if (found) {
			counter++
			fmt.Printf("counter increased in line : %d , Counter : %d\n", i, counter)
		}
	}
